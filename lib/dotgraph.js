...
const DotGraph = (function () {
    ...
    DotGraph.prototype.nodes = function () {
        var k,
            res = [];
        for (k in this.nodesMap) {
            this.nodesMap[k].forEach(n => res.push(n));
        }
        return res;
    };
    ...
    DotGraph.prototype.addNode = function (nodespec) {
        ...
        k = "" + nodespec.represents;
        nodes = this.nodesMap[k];
        if (!Array.isArray(nodes)) {
            this.nodesMap[k] = [node];
        } else {
            nodes.push(node);
        }
        return node;
    ...
    DotGraph.prototype.addPath = function () {
        var args  = slice(arguments),
            ...
            edges = [],
            i, from,
            to    = nodeFor(this.nodesMap, args[0]);
        if (to === undefined) {
            throw new Error("no node for " + (typeof args[0]) + ' ' + args[0]);
        }
        for (i = 1; i < n; i++) {
            from = to;
            to = nodeFor(this.nodesMap, args[i]);
            if (to === undefined) {
    ...
    DotGraph.prototype.toString = function () {
        ...
        this.nodes().forEach(n => {
            this.nodeTemplates.forEach(t => t(n));
            if (n.rank === undefined) {
                nodesInNoRank.push(n);
            } else {
                this.rank(n.rank).addNode(n);
            }
        });
